...
public class ImageEditActivity extends BaseImageEditActivity
    ...
    public void onImageCropped(Bitmap bitmap, Rect cropRect) {
        ...
        if (photoEditorFragment != null) {
            Bundle bundle = photoEditorFragment.getArguments();
            bundle.putBoolean(ImageEditor.EXTRA_IS_CROPPED, true);
            photoEditorFragment.setArguments(bundle);
            photoEditorFragment.reset();
            photoEditorFragment.setImageWithRect(cropRect,bitmap);
            FragmentUtil.popBack(this);
        }
    }
