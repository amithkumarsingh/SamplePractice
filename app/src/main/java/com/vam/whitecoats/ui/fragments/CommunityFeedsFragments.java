...
public class CommunityFeedsFragments extends Fragment implements UiUpdateListener, OnFeedItemClickListener {
    ...
    private void requestForSkillingFiltersData(int page_num) {
        new VolleySinglePartStringRequest(getContext(), Request.Method.POST, RestApiConstants.GET_CONTENT_FEEDS, prepareGetRequestData(page_num), "CONTENT_FEEDS", new OnReceiveResponse() {
            @Override
                        ...
                        if (jsonObject.optString(RestUtils.TAG_STATUS).equalsIgnoreCase(RestUtils.TAG_SUCCESS)) {
                            if (jsonObject.has(RestUtils.TAG_DATA)) {
                                if (jsonObject.optJSONObject(RestUtils.TAG_DATA).has("category_ids")) {
                                    ...
                                    new VolleySinglePartStringRequest(getContext(), Request.Method.POST, RestApiConstants.GET_USER_CATEGORIES_DISTRIBUTION, prepareGetCategoryListRequest(), "GETCONTENTCATERGORYLIST", new OnReceiveResponse() {
                                        @Override
                                        public void onSuccessResponse(String successResponse) {
                                            JSONObject jsonObject = null;
                                            if (successResponse != null) {
    ...
    public void requestForContentFeeds(int pageNum, boolean pullToRefresh, Integer subCategoryId) {
        ...
        new VolleySinglePartStringRequest(getContext(), Request.Method.POST, RestApiConstants.GET_CONTENT_FEEDS, prepareGetCategoryListRequest(pageNum,subCategoryId), navigationFrom, new OnReceiveResponse() {
            @Override
            public void onSuccessResponse(String successResponse) {
                Log.e("communityFeedsResponse",successResponse);
                if (swipeContainer_content_feeds != null) {
                    swipeContainer_content_feeds.setRefreshing(false);
                }
                shimmerContentLayout.setVisibility(View.GONE);
    ...
    private String prepareGetCategoryListRequest(int page_num,int subCategoryId) {
        ...
        Log.e("communityFeedsRequest",reqObj.toString());
        return reqObj.toString();
    }

    public static CommunityFeedsFragments newInstance(String navigatedFrom) {

        Bundle args = new Bundle();

        CommunityFeedsFragments fragment = new CommunityFeedsFragments();
