...
public class CustomImageView extends androidx.appcompat.widget.AppCompatImageView implements AnimationListener {

    private Point coords = new Point();

    public boolean _selected;

    public boolean _delayedSelected;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this._delayedSelected) {
            ...
            canvas.drawRect(0, 0, this.getWidth() - 1f, this.getHeight() - 1f, p);
        }
        canvas.save();
        canvas.rotate(this.currentDegree, this.getWidth() / 2f,this.getHeight() / 2f);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                coords.y = (int) event.getRawY();// + (this.getWidth() / 2);
            ...
            case MotionEvent.ACTION_UP:
                if (this.getParent() instanceof CustomRelativeLayout) {
                    if (((CustomRelativeLayout) this.getParent()).outOfBounds(coords)) {
                        ((CustomRelativeLayout) this.getParent()).removeFrmList(new Point((int) event.getRawX(), (int) event.getRawY()),this);
//                        ((CustomRelativeLayout) this.getParent()).isOverTrash(new Point((int) event.getRawX(), (int) event.getRawY()));
    ...
    public void rotateX(int amount) {
        ...
        this.setLayoutParams(new RelativeLayout.LayoutParams((int) (this.getWidth() * 1), (int) (this.getHeight() * 1)));
        try {
            ((View) this.getParent()).invalidate();
        }catch (Exception e){
    ...
    private Bitmap padBitmap(Bitmap newBm, int widthcrop, int heightcrop) {
        ...
        return Bitmap.createBitmap(pixels, newBm.getWidth() + 2 * widthcrop, newBm.getHeight() + 2 * heightcrop, Config.ARGB_8888);
    }
    ...
    public void setImageResource(int res) {
        ...
        Bitmap tempBitmap = BitmapFactory.decodeResource(getResources(), res);
        this.bitmap = Bitmap.createScaledBitmap(tempBitmap, 100, 100, false);
        setDimensions();
    }
